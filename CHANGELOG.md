...
### feat（新功能）
- 周报数据增加
  - 新增 `#三角洲周报 详细` 命令，支持显示完整详细数据（`showExtra=true`）
...
- 战绩查询功能增强
  - 支持显示队友信息（烽火地带和全面战场）
  - 全面战场模式支持从 `RoomInfo.mpDetailList` 获取队友数据
  - 每页显示战绩数量从10条调整为5条

### style（样式优化）
- 优化物品查询功能（`apps/tools/Object.js`）
  - 二级分类使用中文显示（`secondClassCN`）
- 优化藏品查询渲染模板（`resources/Template/collection/`）
  - 干员皮肤每个卡片使用对应物品图片作为背景（之前所有卡片共用第一个物品的图片）
  - 增加枪皮和典藏枪皮图片大小（从140px增加到180px）
  - 增加军牌图片大小（从80px增加到220px）
  - 增加挂饰图片大小（新增160px规则）
  - 优化干员皮肤遮罩透明度，提升图片可见度
- 优化特勤处信息渲染模板（`resources/Template/placeInfo/`）
  - 重写整体布局和样式设计，采用更简洁现代的设计风格
  - 优化升级条件区域显示，使用网格布局展示条件信息
  - 优化所需物资展示
  - 优化解锁物品库显示
  - 优化等级显示区域
- 优化战绩查询渲染模板（`resources/Template/record/`）
  - 重写整体布局和样式设计，采用更专业的战绩面板风格
  - 优化数据项样式：左对齐显示，添加左侧边框装饰，标签使用透明背景简洁风格
  - 优化状态标签：使用半透明背景和边框，颜色区分（成功绿色、失败红色、退出橙色）
  - 优化击杀统计和救援：改为左右分布布局，击杀明细区域添加左侧竖线装饰
  - 新增队友信息区域：显示队友干员、状态、价值/得分、KDA/击杀、时长、救援等数据
- 优化战绩推送渲染模板（`resources/Template/recordPush/`）
  - 参考战绩查询模板，统一设计风格
  - 优化救援数据显示，提升可读性
- 优化个人数据渲染模板（`resources/Template/personalData/`）

## [1.5.2-2] - 历史版本

### feat（新功能）
- AI评价功能生成的语音支持使用^tts上传来获取语音文件
- 优化藏品查询功能（`apps/info/Collection.js`）
  - 新增物品ID显示，每个藏品显示名称和ID（格式：`物品名称 (ID: 物品ID)`）
  - 新增类型筛选功能，支持 `^藏品 [类型]` 命令（如：`^藏品 干员皮肤`、`^藏品 喷漆`）
  - 支持的类型：干员皮肤、喷漆、挂饰、典藏枪皮、枪皮、载具、头像、军牌
  - 新增 `^藏品 all` 命令，查询所有藏品
  - 新增 `^藏品` 命令（无参数），显示支持的查询类型提示
  - 新增物品图片显示功能，使用合并转发消息展示藏品图片和名称
- 优化特勤处信息查询功能（`apps/info/PlaceInfo.js`）
  - 新增 `^特勤处信息 all` 命令，查询所有设施（每个类型一个合并转发，每个等级一个图片）
  - 新增 `^特勤处信息 [类型]` 命令，查询指定类型的所有等级（如：`^特勤处信息 仓库`）
  - 新增 `^特勤处信息 [类型] [等级]` 命令，查询指定类型和等级（如：`^特勤处信息 仓库 1`）
  - 优化命令提示，无参数时显示正确的命令格式和可用类型
  - 优化等级查询逻辑，请求的等级不存在时自动返回最高可用等级
...
- 优化特勤处信息渲染模板（`resources/Template/placeInfo/`）
  ...
  - 优化需求物资展示
  - 优化解锁效果区域显示
- 新增藏品查询渲染模板（`resources/Template/collection/`）
